...
# Settlement-station ASOS IDs (IEM) — the NWS/METAR source markets settle on.
STATION = {
    "atlanta": ("ATL", "GA_ASOS"), "dallas": ("DAL", "TX_ASOS"),
    "austin": ("AUS", "TX_ASOS"), "seattle": ("SEA", "WA_ASOS"),
    "toronto": ("YYZ", "CA_ASOS"), "sao_paulo": ("SBGR", "BR__ASOS"),
    "buenos_aires": ("SAEZ", "AR__ASOS"), "london": ("EGLC", "GB__ASOS"),
    "paris": ("LFPB", "FR__ASOS"), "madrid": ("LEMD", "ES__ASOS"),
}
_iem_cache = {}


def _iem_station_series(city):
    if city in _iem_cache:
        return _iem_cache[city]
    st, net = STATION[city]
    ...
    try:
        ...
        for r in d.get("data", []):
            if r.get("max_tmpf") is not None:
                series[r.get("date")] = float(r["max_tmpf"])
    except Exception as e:
        print(f"  IEM fetch failed {city}: {e}", file=sys.stderr)
    _iem_cache[city] = series
    return series


def era5_obs_high(target_day):
    """{city: STATION observed daily-high in city unit} (name kept for reuse)."""
    out = {}
    for city, (lat, lon, unit, *_ ) in CITIES.items():
        f = _iem_station_series(city).get(target_day.isoformat())
        out[city] = None if f is None else (f if unit == "F" else (f - 32) * 5 / 9)
    return out
