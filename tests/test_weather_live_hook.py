"""G2-2 tests (v2 aggressive-hybrid, 2026-07-09): the flag-gated live hook
(resolve_weather_live). execute_aggressive_hybrid is always MOCKED — no network,
no real order. Covers: flag-off=paper, Kalshi/non-polymarket can't go live,
missing-token skip, confirmed-take economics + per-trade cap + 15-share bump,
pure-rest = 'rested' (no row), hard-fail = 'skip', and the daily kill-switch.
"""
import types
from datetime import datetime

import pytest
from sqlalchemy import create_engine
from sqlalchemy.orm import sessionmaker

from backend.models.database import Base, Trade
from backend.core.scheduler import resolve_weather_live


def _signal(direction="yes", platform="polymarket", token_yes="YESTOK", token_no="NOTOK",
            market_type="weather", model_yes=0.60):
    market = types.SimpleNamespace(
        platform=platform, market_type=market_type, market_id="2400000", slug="slug",
        token_id_yes=token_yes, token_id_no=token_no,
        model_probability=model_yes, threshold_f=90.0, direction="above",
        metric="high", city_name="Chicago", city_key="nyc",
    )
    return types.SimpleNamespace(
        market=market, direction=direction, edge=0.20,
        market_probability=0.40, model_probability=model_yes, suggested_size=50.0,
    )


def _settings(live=True, cap=15.0, stop=10.0):
    return types.SimpleNamespace(
        WEATHER_LIVE_TRADING=live,
        WEATHER_LIVE_MAX_TRADE_USD=cap,
        WEATHER_LIVE_DAILY_LOSS_STOP_USD=stop,
        WEATHER_LIVE_MAX_TOTAL_EXPOSURE_USD=1000.0,
        WEATHER_LIVE_MIN_EDGE_FLOOR=0.05,
        WEATHER_LIVE_CITIES="nyc",
    )


class _MockTrader:
    """Records execute_aggressive_hybrid calls; returns a preset lifecycle dict
    (or None for a hard failure)."""
    def __init__(self, result):
        self.result = result
        self.calls = []

    def execute_aggressive_hybrid(self, token_id, size_usd, limit_price):
        self.calls.append((token_id, size_usd, limit_price))
        if self.result is None:
            return None
        r = dict(self.result)
        r.setdefault("price", limit_price)
        return r


def _db():
    eng = create_engine("sqlite:///:memory:")
    Base.metadata.create_all(eng)
    return sessionmaker(bind=eng)()


def _seed_loss(db, pnl, order_id="OLD", when=None):
    ts = when or datetime.utcnow()
    db.add(Trade(market_ticker="m", platform="polymarket", market_type="weather",
                 direction="yes", entry_price=0.4, size=2.0, order_id=order_id,
                 settled=True, pnl=pnl, timestamp=ts, settlement_time=ts))
    db.commit()


# A confirmed TAKE (immediate fill) and a pure REST (0 take, order resting).
_TAKE = {"order_id": "OID1", "fill_price": 0.55, "filled_shares": 15.5,
         "filled_cost": 8.53, "resting_shares": 0.0, "status": "matched"}
_REST = {"order_id": "OID2", "fill_price": None, "filled_shares": 0.0,
         "filled_cost": 0.0, "resting_shares": 16.0, "status": "live"}


# ── flag-off / defense-in-depth ──────────────────────────────────────────────
def test_flag_off_is_paper():
    d = resolve_weather_live(_signal(), 50.0, 0.40, None, _settings(live=False), None)
    assert d.action == "paper"
    assert d.size == 50.0 and d.entry_price == 0.40 and d.order_id is None


def test_kalshi_can_never_go_live():
    d = resolve_weather_live(_signal(platform="kalshi"), 50.0, 0.40, None, _settings(live=True), None)
    assert d.action == "paper"


def test_non_weather_can_never_go_live():
    d = resolve_weather_live(_signal(market_type="btc"), 50.0, 0.40, None, _settings(live=True), None)
    assert d.action == "paper"


# ── guards ───────────────────────────────────────────────────────────────────
def test_missing_token_id_skips():
    t = _MockTrader(_TAKE)
    d = resolve_weather_live(_signal(token_yes="", direction="yes"), 50.0, 0.40,
                             _db(), _settings(), lambda: t)
    assert d.action == "skip"
    assert t.calls == []  # never even attempted an order


# ── take economics + cap + min-size bump ─────────────────────────────────────
def test_confirmed_take_writes_actual_economics_and_caps_size():
    t = _MockTrader(_TAKE)
    d = resolve_weather_live(_signal(), 50.0, 0.40, _db(), _settings(cap=15.0), lambda: t)
    assert d.action == "fill"
    assert d.entry_price == 0.55      # ACTUAL take price, not the 0.40 quote
    assert d.size == 8.53             # ACTUAL cost of the take
    assert d.order_id == "OID1"
    assert t.calls[0][1] == 15.0      # size passed to hybrid clamped at the $15 cap
    # limit_price passed = model YES 0.60 - 0.05 floor = 0.55
    assert t.calls[0][2] == pytest.approx(0.55)


def test_pure_rest_returns_rested_no_row():
    t = _MockTrader(_REST)
    d = resolve_weather_live(_signal(), 50.0, 0.40, _db(), _settings(), lambda: t)
    assert d.action == "rested"       # order is resting; no Trade row yet
    assert d.order_id == "OID2"
    assert len(t.calls) == 1


def test_hard_failure_returns_skip():
    t = _MockTrader(None)             # execute_aggressive_hybrid returned None
    d = resolve_weather_live(_signal(), 50.0, 0.40, _db(), _settings(), lambda: t)
    assert d.action == "skip"
    assert len(t.calls) == 1


def test_guard_skip_when_cap_too_small():
    # NO side, model_yes 0.60 → limit 0.35; cap $5 < min_for_15 $5.30 → guard skip.
    t = _MockTrader(_TAKE)
    d = resolve_weather_live(_signal(direction="no"), 50.0, 0.40, _db(),
                             _settings(cap=5.0), lambda: t)
    assert d.action == "guard_skip"
    assert t.calls == []


# ── kill-switch ──────────────────────────────────────────────────────────────
def test_killswitch_halts_at_or_above_stop():
    db = _db()
    _seed_loss(db, pnl=-12.0)
    d = resolve_weather_live(_signal(), 50.0, 0.40, db, _settings(stop=10.0),
                             lambda: _MockTrader(_TAKE))
    assert d.action == "halt"


def test_killswitch_allows_below_stop():
    db = _db()
    _seed_loss(db, pnl=-5.0)
    d = resolve_weather_live(_signal(), 50.0, 0.40, db, _settings(stop=10.0),
                             lambda: _MockTrader(_TAKE))
    assert d.action == "fill"


def test_killswitch_ignores_paper_losses():
    db = _db()
    _seed_loss(db, pnl=-99.0, order_id=None)
    d = resolve_weather_live(_signal(), 50.0, 0.40, db, _settings(stop=10.0),
                             lambda: _MockTrader(_TAKE))
    assert d.action == "fill"


# ── P0-1 pause honesty (2026-07-19) ──────────────────────────────────────────
def test_live_deployment_paused_halts_no_sim_row():
    # LIVE deployment + flag off → HALT (no paper-sim rows into the live DB).
    s = _settings(live=False); s.WEATHER_LIVE_DEPLOYMENT = True
    d = resolve_weather_live(_signal(), 50.0, 0.40, _db(), s, lambda: None)
    assert d.action == "halt_paused"


def test_paper_deployment_flag_off_still_paper():
    # PAPER deployment (no WEATHER_LIVE_DEPLOYMENT) + flag off → paper (unchanged).
    s = _settings(live=False)
    d = resolve_weather_live(_signal(), 50.0, 0.40, _db(), s, lambda: None)
    assert d.action == "paper"


def test_live_deployment_flag_on_still_trades():
    # LIVE deployment + flag ON → normal live path (not halted by P0-1).
    s = _settings(live=True); s.WEATHER_LIVE_DEPLOYMENT = True
    t = _MockTrader(_TAKE)
    d = resolve_weather_live(_signal(), 50.0, 0.40, _db(), s, lambda: t)
    assert d.action == "fill"
